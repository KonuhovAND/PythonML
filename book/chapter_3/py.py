...
X_train,X_test,y_train,y_test = sklearn.model_selection.train_test_split(X_data,y_target,test_size=.2,random_state=42)
...
print(f"precision_score is {precision_score(y_test_7,y_pr)}") # precision_score is 1.0
print(f"recall is {recall_score(y_test_7,y_pr)}") # recall is 0.970
print(f"accuracy_score is {accuracy_score(y_test_7,y_pr)}") #accuracy_score is 0.99722
print(f"root_mean_squared_error is {root_mean_squared_error(y_test_7,y_pr)}") #root_mean_squared_error is 0.0527

from sklearn.model_selection import cross_val_score

sdg_clf = SGDClassifier(random_state=42)

print(f"Cross_val_score is {cross_val_score(sdg_clf,X_train,y_train_7,cv=3,scoring='accuracy')}")
...
print(f"Cross_val_score is {cross_val_score(dummy,X_train,y_train_7,cv=3,scoring='accuracy')}")
...
print(f"Confusion matrix of prediction is:\n {confusion_matrix(y_train_7,y_prediction)}")
#>>> [[1284    8]
#>>> [   6  139]]

from sklearn.metrics import roc_curve

y_prediction = cross_val_predict(sdg_clf,X_train,y_train_7,cv=3,method='decision_function')
